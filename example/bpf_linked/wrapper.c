...
struct
{
        ...
        __type(key, __u32);
        __type(value, __u64);
        __uint(max_entries, TTC_MAP_SIZE);
} time_to_complete SEC(".maps");

static __u32 is_map_init = 0;
...
static __always_inline void update_time(void *map, __u64 new_value)
{
        ...
        if(!is_map_init) {
                push(map, 0, 1);
                push(map, 1, new_value);
                is_map_init++;
        } else {
                is_map_init++;
                if(is_map_init >= TTC_MAP_SIZE) return;
                push(map, is_map_init, new_value);
                push(map, 0, is_map_init);
        }
}
